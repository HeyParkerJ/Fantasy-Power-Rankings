...
const renderRankings = (rankings, users) => {
  ...
  // Processing
  placementsArray = RankingsUtils.appendEmojisToRankingsArray(
    placementsArray,
    users
  );
  ...
  placementsArray.forEach((r, index) => {
    ...
    let rankingsAverageRounded =
      Math.round(parseInt(r.rankingsAverage * 100)) / 100;

    // Output
    rankingRows.push(
      <div key={index + 1}>
        <strong>{r.tieAdjustedRanking}:</strong> {r.username} &nbsp;{' '}
        <span>{emoji.getUnicode(r.emoji)}</span> &nbsp; (
        {rankingsAverageRounded})
      </div>
    );
  });
